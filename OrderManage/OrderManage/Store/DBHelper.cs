...
using OrderManage.Util;//这个是添加的引用
/// 
/// 这个是操作Access数据库的类,提供2个方法,getSQLTable获取表,executeSQL执行所需要的SQL语句
/// 
namespace OrderManage.Store
{
    public class DBHelper
    {
        protected static string connectionString = string.Format("Provider=Microsoft.Jet.OleDb.4.0;Data Source={0}data.mdb;Jet OleDb:DataBase Password='';",System.Environment.CurrentDirectory+@"\");
        public static string connectionStr()
        {
            string strConnection = "Provider=Microsoft.Jet.OleDb.4.0;";
            return strConnection += string.Format("Data Source={0}data.mdb;Jet OleDb:DataBase Password='';", System.Environment.CurrentDirectory + @"\");
        }
        public DBHelper()
        {
        }
        ...
        public static int ExecuteSql(string SQLString, string content)
        {
            using (OleDbConnection connection = new OleDbConnection(connectionString))
            {
                ...
                OleDbCommand cmd = connection.CreateCommand();
                cmd.CommandText = SQLString;
                OleDbParameter myParameter = new OleDbParameter("@content",OleDbType.LongVarChar);
        ...
        public static int ExecuteSqlInsertImg(string strSQL, byte[] fs)
        {
            using (OleDbConnection connection = new OleDbConnection(connectionString))
            {
                ...
                OleDbCommand cmd = connection.CreateCommand();
                cmd.CommandText = strSQL;
                OleDbParameter myParameter = new OleDbParameter("@fs", OleDbType.VarChar);//原来是图片
        ...
        public static void ExecuteSqlTran(Hashtable OleDbStringList)
        {
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                conn.Open();
